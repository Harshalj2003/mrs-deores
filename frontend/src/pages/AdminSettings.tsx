            ...
            <div className="max-w-4xl mx-auto">
                ...
                <div className="space-y-6">
                    ...
                    <SectionCard icon={<Megaphone className="h-5 w-5" />} title="Announcement Bar" subtitle="Top banner across the website">
                        <div className="space-y-6">
                            <div className="flex items-center justify-between p-4 bg-neutral-light/50 rounded-2xl border border-gray-100">
                                <div>
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                                ...
                                <Field label="Animation Effect" hint="Visual feedback to capture attention">
                                    <div className="flex gap-2 flex-wrap">
                                        {[
                                            ...
                                            <button
                                                key={val}
                                                type="button"
                                                onClick={() => set('announcement_animation', val)}
                                                className={`px-3 py-1.5 rounded-xl text-[10px] font-bold transition-all ${get('announcement_animation', 'none') === val ? 'bg-primary text-white shadow-lg shadow-primary/20' : 'bg-neutral-light/50 text-gray-600 hover:bg-primary/10'}`}
                                            >
                                                {label}
                                            </button>
                                        ))}
                                ...
                                <Field label="Bar Thickness" hint="Sm: 1.5rem | Md: 2.5rem | Lg: 3.5rem">
                                    <div className="flex gap-2">
                                        {[
                                            ...
                                            <button
                                                key={val}
                                                type="button"
                                                onClick={() => set('announcement_height', val)}
                                                className={`px-3 py-1.5 rounded-xl text-[10px] font-bold transition-all ${get('announcement_height', 'md') === val ? 'bg-primary text-white shadow-lg shadow-primary/20' : 'bg-neutral-light/50 text-gray-600 hover:bg-primary/10'}`}
                                            >
                                                {label}
                                            </button>
                                        ))}
                                ...
                                <Field label="Layout Style" hint="Full width or floating with margins">
                                    <div className="flex gap-2">
                                        {[
                                            ['full', 'Full Width'],
                                            ['boxed', 'Boxed / Floating']
                                        ].map(([val, label]) => (
                                            <button
                                                key={val}
                                                type="button"
                                                onClick={() => set('announcement_width', val)}
                                                className={`px-3 py-1.5 rounded-xl text-[10px] font-bold transition-all ${get('announcement_width', 'full') === val ? 'bg-primary text-white shadow-lg shadow-primary/20' : 'bg-neutral-light/50 text-gray-600 hover:bg-primary/10'}`}
                                            >
                                                {label}
                                            </button>
                                        ))}
                        ...
                        <div className="space-y-6">
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
                                ...
                                <Field label="Button Color">
                                    <div className="flex items-center gap-3">
                                        <input type="color" value={get('whatsapp_color', '#25D366')} onChange={e => set('whatsapp_color', e.target.value)} className="h-10 w-14 rounded-xl border border-gray-200 cursor-pointer" />
                                        <Input value={get('whatsapp_color', '#25D366')} onChange={v => set('whatsapp_color', v)} />
                                    </div>
                                </Field>
                                <Field label="Custom Icon URL" hint="Optional. Overrides the default WhatsApp logo.">
                                    <Input value={get('whatsapp_icon_url')} onChange={v => set('whatsapp_icon_url', v)} placeholder="https://..." />
                                </Field>
                    ...
                    {/* 6.2 FAQ WhatsApp Contact */}
                    <SectionCard icon={<MessageSquare className="h-5 w-5 text-secondary" />} title="FAQ WhatsApp Button" subtitle="Customize the WhatsApp contact button shown inside the Knowledge Base / FAQ section">
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
                            <Field label="FAQ WhatsApp Number" hint="Include country code (e.g., 919876543210)">
                                <Input value={get('faq_whatsapp_number', '918459424840')} onChange={v => set('faq_whatsapp_number', v)} placeholder="918459424840" />
                            </Field>
                            <Field label="Initial Message" hint="Default text when user clicks the FAQ button">
                                <Input value={get('faq_whatsapp_message', "Hello Mrs. Deores! I have a question about your authentic products.")} onChange={v => set('faq_whatsapp_message', v)} />
                            </Field>
                        </div>
                    </SectionCard>
                    ...
                    >
                        <div className="space-y-8">
                            ...
                            <div className="space-y-4">
                                ...
                                {/* Live Previews — Category */}
                                <div className="grid grid-cols-1 md:grid-cols-2 gap-5 mt-4">
                                    {/* Mobile Preview */}
                                    <div className="p-4 bg-neutral-light/50 rounded-2xl border border-gray-200">
                                        <p className="text-[10px] font-black text-gray-400 uppercase tracking-widest mb-3">Preview — Mobile ({get('grid_categories_mobile', '2')} cols)</p>
                                        <div
                                            className="gap-2 grid transform origin-top left"
                                            style={{ gridTemplateColumns: `repeat(${get('grid_categories_mobile', '2')}, minmax(0, 1fr))` }}
                                        >
                                            {Array.from({ length: Math.min(6, parseInt(get('grid_categories_mobile', '2')) * 2) }).map((_, i) => (
                                                <div key={i} className="bg-primary/10 rounded-xl aspect-square flex items-center justify-center text-primary/30 font-black text-[10px]">
                                                    Cat {i + 1}
                                                </div>
                                            ))}
                                        </div>
                                    ...
                                    {/* Desktop Preview */}
                                    <div className="p-4 bg-neutral-light/50 rounded-2xl border border-gray-200 hidden md:block">
                                        <p className="text-[10px] font-black text-gray-400 uppercase tracking-widest mb-3">Preview — Desktop ({get('grid_categories_desktop', '4')} cols)</p>
                                        <div
                                            className="gap-2 grid transform origin-top left"
                                            style={{ gridTemplateColumns: `repeat(${get('grid_categories_desktop', '4')}, minmax(0, 1fr))` }}
                                        >
                                            {Array.from({ length: Math.min(8, parseInt(get('grid_categories_desktop', '4')) * 2) }).map((_, i) => (
                                                <div key={i} className="bg-primary/10 rounded-xl aspect-[4/3] flex items-center justify-center text-primary/30 font-black text-[10px]">
                                                    Cat {i + 1}
                                                </div>
                                            ))}
                                        </div>
                            ...
                            <div className="space-y-4">
                                ...
                                {/* Live Previews — Products */}
                                <div className="grid grid-cols-1 md:grid-cols-2 gap-5 mt-4">
                                    {/* Mobile Preview */}
                                    <div className="p-4 bg-neutral-light/50 rounded-2xl border border-gray-200">
                                        <p className="text-[10px] font-black text-gray-400 uppercase tracking-widest mb-3">Preview — Mobile ({get('grid_products_mobile', '2')} cols)</p>
                                        <div
                                            className="gap-2 grid transform origin-top left"
                                            style={{ gridTemplateColumns: `repeat(${get('grid_products_mobile', '2')}, minmax(0, 1fr))` }}
                                        >
                                            {Array.from({ length: Math.min(4, parseInt(get('grid_products_mobile', '2')) * 2) }).map((_, i) => (
                                                <div key={i} className="bg-secondary/15 rounded-xl overflow-hidden pb-1">
                                                    <div className="aspect-square bg-secondary/20 flex items-center justify-center text-secondary/40 font-black text-[10px]">IMG</div>
                                                    <div className="p-1.5">
                                    ...
                                    {/* Desktop Preview */}
                                    <div className="p-4 bg-neutral-light/50 rounded-2xl border border-gray-200 hidden md:block">
                                        <p className="text-[10px] font-black text-gray-400 uppercase tracking-widest mb-3">Preview — Desktop ({get('grid_products_desktop', '3')} cols)</p>
                                        <div
                                            className="gap-3 grid transform origin-top left"
                                            style={{ gridTemplateColumns: `repeat(${get('grid_products_desktop', '3')}, minmax(0, 1fr))` }}
                                        >
                                            {Array.from({ length: Math.min(6, parseInt(get('grid_products_desktop', '3')) * 2) }).map((_, i) => (
                                                <div key={i} className="bg-secondary/15 rounded-xl overflow-hidden pb-2">
